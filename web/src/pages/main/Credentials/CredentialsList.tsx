...
import {
  credentialListTableHeadCells,
  credentialTypeMap,
} from '../../../providers/CredentialProvider'
...
const CredentialsList = () => {
  ...
  const [rows, setRows] = useState<CredentialDataProps[]>([])

  const { data, setData } = useAxiosGet<CredentialDataProps[]>(
  ...
  const handleSearch = (
    ...
    if (newString) {
      const newRows = rows.filter((row: KeyedObject) => {
        ...
        const properties = ['id', 'name', 'username', 'credential_type']
  ...
  const deleteCredential = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/credentials', { data: selected })
      .then((response) => {
        if (response.status === 204) {
          setData(
            data!.filter((credential) => !selected.includes(credential.id)),
          )
          dispatch(
            openSnackbar({
              open: true,
  ...
  return (
    <MainCard
      title={<FormattedMessage id="credentials.list.title" />}
      content={false}
    >
      <CardContent>
        <Grid
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'credentials.placeholders.searchCredentials',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                credentialListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<CredentialDataProps>(
              rows,
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="left">
                      {credentialTypeMap.get(row.credential_type)?.icon}
                      <FormattedMessage
                        id={credentialTypeMap.get(row.credential_type)?.label}
                      />
                    </TableCell>
