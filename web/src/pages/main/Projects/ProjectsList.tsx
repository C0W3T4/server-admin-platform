...
const ProjectsList = () => {
  ...
  const [rows, setRows] = useState<ProjectDataProps[]>([])

  const { data, setData } = useAxiosGet<ProjectDataProps[]>(
  ...
  const deleteProject = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/projects', { data: selected })
      .then((response) => {
        if (response.status === 204) {
          setData(data!.filter((project) => !selected.includes(project.id)))
          dispatch(
            openSnackbar({
              open: true,
    ...
    await api
      ...
      .then((response) => {
        if (response.status === 200) {
          if (data) {
            setData(
              data.map((project: ProjectDataProps) =>
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'projects.placeholders.searchProjects',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                projectListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<ProjectDataProps>(rows, getComparator(order, orderBy))
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="left">
                      {projectStatusMap.get(row.project_status)?.icon}
                      <FormattedMessage
                        id={projectStatusMap.get(row.project_status)?.label}
                      />
                    </TableCell>
                    <TableCell align="left">
                      {toolsMap.get(row.tool)?.icon}
                      <FormattedMessage id={toolsMap.get(row.tool)?.label} />
                    </TableCell>
                    <TableCell align="left">
                      {
                        sourceControlMap.get(row.source_control_credential_type)
                          ?.icon
                      }
                      <FormattedMessage
                        id={
                          sourceControlMap.get(
                            row.source_control_credential_type,
                          )?.label
                        }
                      />
                    </TableCell>
                    <TableCell align="center" sx={{ pr: 3 }}>
                      ...
                      >
                        <IconButton
                          size="large"
                          onClick={(event) => handleUpdateRepo(event, row.id)}
                        >
