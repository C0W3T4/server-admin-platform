...
import { TbCirclePlus, TbRocket } from 'react-icons/tb'
...
const TemplatesList = () => {
  ...
  const [rows, setRows] = useState<TemplateDataProps[]>([])

  const { data, setData } = useAxiosGet<TemplateDataProps[]>(
    `api/assigns/users-templates/${user?.id}/templates`,
  )
  ...
  const handleSearch = (
    ...
    if (newString) {
      const newRows = rows.filter((row: KeyedObject) => {
        ...
        const properties = ['id', 'name', 'launch_type']
  ...
  const deleteTemplate = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/templates', { data: selected })
      .then((response) => {
        if (response.status === 204) {
          setData(data!.filter((template) => !selected.includes(template.id)))
          dispatch(
            openSnackbar({
              open: true,
  ...
  return (
    <MainCard
      title={<FormattedMessage id="templates.list.title" />}
      content={false}
    >
      <CardContent>
        <Grid
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'templates.placeholders.searchTemplate',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                templateListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<TemplateDataProps>(rows, getComparator(order, orderBy))
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="left">
                      {launchTypeMap.get(row.launch_type)?.icon}
                      <FormattedMessage
                        id={launchTypeMap.get(row.launch_type)?.label}
                      />
                    </TableCell>
                    <TableCell align="center" sx={{ pr: 3 }}>
                      ...
                      >
                        <IconButton
                          color="info"
                          size="large"
                          onClick={(event) => handleLaunchJob(event, row)}
                        >
                          <TbRocket size={16} />
                        </IconButton>
                      </Tooltip>
                      ...
                      >
                        <IconButton
                          size="large"
                          onClick={(event) =>
                            handleUpdateRepo(event, row.project.id)
                          }
                        >
                      ...
                      >
                        <IconButton
                          size="large"
                          onClick={(event) =>
                            handleSyncInventory(event, row.inventory.id)
                          }
                        >
