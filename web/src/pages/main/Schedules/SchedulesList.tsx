...
const SchedulesList = () => {
  ...
  const [rows, setRows] = useState<ScheduleDataProps[]>([])

  const { data, setData } = useAxiosGet<ScheduleDataProps[]>('api/schedules')
  ...
  const handleSearch = (
    ...
    if (newString) {
      const newRows = rows.filter((row: KeyedObject) => {
        ...
        const properties = ['id', 'name', 'schedule_type']
  ...
  const deleteSchedule = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/schedules', { data: selected })
      .then((response) => {
        if (response.status === 204) {
          setData(data!.filter((schedule) => !selected.includes(schedule.id)))
          dispatch(
            openSnackbar({
              open: true,
  ...
  return (
    <MainCard
      title={<FormattedMessage id="schedules.list.title" />}
      content={false}
    >
      <CardContent>
        <Grid
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'schedules.placeholders.searchSchedule',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                scheduleListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<ScheduleDataProps>(rows, getComparator(order, orderBy))
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="left">
                      {scheduleTypeMap.get(row.schedule_type)?.icon}
                      <FormattedMessage
                        id={scheduleTypeMap.get(row.schedule_type)?.label}
                      />
                    </TableCell>
