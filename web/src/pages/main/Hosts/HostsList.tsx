...
import {
  hostListTableHeadCells,
  hostStatusMap,
} from '../../../providers/HostProvider'
...
const HostsList = () => {
  ...
  const [rows, setRows] = useState<HostDataProps[]>([])

  const { data, setData } = useAxiosGet<HostDataProps[]>(
  ...
  const handleSearch = (
    ...
    if (newString) {
      const newRows = rows.filter((row: KeyedObject) => {
        ...
        const properties = ['id', 'hostname', 'ipv4', 'host_status']
  ...
  const deleteHost = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/hosts', { data: selected })
      .then((response) => {
        if (response.status === 204) {
          setData(data!.filter((host) => !selected.includes(host.id)))
          dispatch(
            openSnackbar({
              open: true,
  ...
  return (
    <MainCard
      title={<FormattedMessage id="hosts.list.title" />}
      content={false}
    >
      <CardContent>
        <Grid
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'hosts.placeholders.searchHosts',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                hostListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<HostDataProps>(rows, getComparator(order, orderBy))
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="left">
                      {hostStatusMap.get(row.host_status)?.icon}
                      <FormattedMessage
                        id={hostStatusMap.get(row.host_status)?.label}
                      />
                    </TableCell>
                    <TableCell align="center" sx={{ pr: 3 }}>
                      <Tooltip
                        TransitionComponent={Zoom}
                        title={
                          <FormattedMessage id="tooltips.updateHostStatus" />
                        }
                      >
                        <IconButton
                          size="large"
                          onClick={(event) =>
                            handleUpdateHostStatus(event, row.id)
                          }
                        >
