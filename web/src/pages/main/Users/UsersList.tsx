...
const UsersList = () => {
  ...
  const { data, setData } = useAxiosGet<UserProfile[]>('api/users')
  ...
  const deleteUser = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/users', { data: selected })
      .then((response) => {
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'users.placeholders.searchUsers',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                userListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<UserProfile>(rows, getComparator(order, orderBy))
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="left">
                      {userTypeMap.get(row.user_type)?.icon}
                      <FormattedMessage
                        id={userTypeMap.get(row.user_type)?.label}
                      />
                    </TableCell>
                    <TableCell align="left">
