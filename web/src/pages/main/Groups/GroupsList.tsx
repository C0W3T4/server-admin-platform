...
import { groupListTableHeadCells } from '../../../providers/GroupProvider'
...
const GroupsList = () => {
  ...
  const [rows, setRows] = useState<GroupDataProps[]>([])

  const { data, setData } = useAxiosGet<GroupDataProps[]>(
  ...
  const deleteGroup = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/groups', { data: selected })
      .then((response) => {
        if (response.status === 204) {
          setData(data!.filter((group) => !selected.includes(group.id)))
          dispatch(
            openSnackbar({
              open: true,
  ...
  return (
    <MainCard
      title={<FormattedMessage id="groups.list.title" />}
      content={false}
    >
      <CardContent>
        <Grid
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'groups.placeholders.searchGroups',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                groupListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<GroupDataProps>(rows, getComparator(order, orderBy))
