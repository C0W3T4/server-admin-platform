...
const InventoriesList = () => {
  ...
  const [rows, setRows] = useState<InventoryDataProps[]>([])

  const { data, setData } = useAxiosGet<InventoryDataProps[]>(
  ...
  const handleSearch = (
    ...
    if (newString) {
      const newRows = rows.filter((row: KeyedObject) => {
        ...
        const properties = ['id', 'name', 'inventory_file', 'inventory_status']
  ...
  const deleteInventory = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/inventories', { data: selected })
      .then((response) => {
        if (response.status === 204) {
          setData(data!.filter((inventory) => !selected.includes(inventory.id)))
          dispatch(
            openSnackbar({
              open: true,
    ...
    await api
      ...
      .then((response) => {
        if (response.status === 200) {
          if (data) {
            setData(
              data.map((inventory: InventoryDataProps) =>
                inventory.id === response.data.id
                  ? { ...response.data }
                  : inventory,
              ),
            )
  ...
  return (
    <MainCard
      title={<FormattedMessage id="inventories.list.title" />}
      content={false}
    >
      <CardContent>
        <Grid
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'inventories.placeholders.searchInventories',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                inventoryListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<InventoryDataProps>(rows, getComparator(order, orderBy))
              ...
              .map((row, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="left">{row.inventory_file}</TableCell>
                    <TableCell align="left">
                      {inventoryStatusMap.get(row.inventory_status)?.icon}
                      <FormattedMessage
                        id={inventoryStatusMap.get(row.inventory_status)?.label}
                      />
                    </TableCell>
                    <TableCell align="center" sx={{ pr: 3 }}>
                      ...
                      >
                        <IconButton
                          size="large"
                          onClick={(event) =>
                            handleSyncInventory(event, row.id)
                          }
                        >
