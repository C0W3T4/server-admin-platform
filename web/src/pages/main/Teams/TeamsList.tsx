...
import { teamListTableHeadCells } from '../../../providers/TeamProvider'
...
const TeamsList = () => {
  ...
  const [rows, setRows] = useState<TeamDataProps[]>([])

  const { data, setData } = useAxiosGet<TeamDataProps[]>(
    'api/assigns/users-teams',
  )
  ...
  const deleteTeam = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/teams', { data: selected })
      .then((response) => {
        if (response.status === 204) {
          setData(data!.filter((team) => !selected.includes(team.id)))
          dispatch(
            openSnackbar({
              open: true,
  ...
  return (
    <MainCard
      title={<FormattedMessage id="teams.list.title" />}
      content={false}
    >
      <CardContent>
        <Grid
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'teams.placeholders.searchTeams',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                teamListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<TeamDataProps>(rows, getComparator(order, orderBy))
