...
import { inventorySchedulesListTableHeadCells } from '../../../../providers/InventoryProvider'
...
import { FormInventorySchedules } from '../../../forms/inventory/FormInventorySchedules'
...
interface InventorySchedulesListProps {
  inventoryData: InventoryDataProps | null
}

export const InventorySchedulesList = ({
  inventoryData,
}: InventorySchedulesListProps) => {
  ...
  const [orderBy, setOrderBy] = useState<string>('inventory_schedule_id')
  const [selected, setSelected] = useState<string[]>([])
  const [page, setPage] = useState<number>(0)
  const [rowsPerPage, setRowsPerPage] = useState<number>(5)
  const [search, setSearch] = useState<string>('')
  const [rows, setRows] = useState<InventorySchedulesDataProps[]>([])
  const [openDialog, setOpenDialog] = useState<boolean>(false)
  const [scheduleInfoData, setScheduleInfoData] = useState<
    JobsScheduleInfoDataProps[]
  >([])

  const { data, setData } = useAxiosGet<InventorySchedulesDataProps[]>(
    `api/assigns/inventories-schedules/${id}/schedules`,
  )
    ...
    if (newString) {
      const newRows = rows.filter((row: KeyedObject) => {
        ...
        properties.forEach((property) => {
          if (
            property === 'inventory_schedule_id' ||
            property === 'next_date'
          ) {
            if (
              row[property]
  ...
  const handleSelectAllClick = (event?: ChangeEvent<HTMLInputElement>) => {
    if (event?.target.checked) {
      const newSelectedId = rows.map((row) => row.inventory_schedule_id)
  ...
  const deleteSchedule = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/assigns/inventories-schedules', { data: selected })
      .then((response) => {
        if (response.status === 204) {
          setData(
            data!.filter(
              (inventorySchedule) =>
                !selected.includes(inventorySchedule.inventory_schedule_id),
            ),
          )
          dispatch(
            openSnackbar({
  ...
  const getScheduleInfo = async () => {
    await api
      .get(`api/assigns/inventories-schedules/${id}/schedules/info`)
  ...
  return (
    <MainCard
      title={`${inventoryData!.name} -> ${formatMessage({
        id: 'inventories.labels.schedules',
      })}`}
      content={false}
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'inventories.placeholders.searchSchedules',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                inventorySchedulesListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<InventorySchedulesDataProps>(
              rows,
              ...
              .map((row, index) => {
                ...
                const isItemSelected = isSelected(row.inventory_schedule_id)
                ...
                return (
                  ...
                  >
                    <TableCell
                      ...
                      sx={{ pl: 3 }}
                      onClick={(event) =>
                        handleClick(event, row.inventory_schedule_id)
                      }
                    >
                      <Checkbox
                        color="primary"
