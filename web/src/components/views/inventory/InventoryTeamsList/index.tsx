...
import { inventoryTeamsListTableHeadCells } from '../../../../providers/InventoryProvider'
import { ArrangementOrder, KeyedObject } from '../../../../types'
import { TeamsInventoryDataProps } from '../../../../types/access'
...
import { FormInventoryTeams } from '../../../forms/inventory/FormInventoryTeams'

interface InventoryTeamsListProps {
  inventoryData?: InventoryDataProps | null
}

export const InventoryTeamsList = ({
  inventoryData,
}: InventoryTeamsListProps) => {
  ...
  const [orderBy, setOrderBy] = useState<string>('team_inventory_id')
  const [selected, setSelected] = useState<string[]>([])
  const [page, setPage] = useState<number>(0)
  const [rowsPerPage, setRowsPerPage] = useState<number>(5)
  const [search, setSearch] = useState<string>('')
  const [rows, setRows] = useState<TeamsInventoryDataProps[]>([])
  const [openDialog, setOpenDialog] = useState<boolean>(false)

  const { data, setData } = useAxiosGet<TeamsInventoryDataProps[]>(
    `api/assigns/teams-inventories/${id}/teams`,
  )
  ...
  const handleSearch = (
    ...
    if (newString) {
      const newRows = rows.filter((row: KeyedObject) => {
        ...
        const properties = ['team_inventory_id', 'name', 'description']
        let containsQuery = false

        properties.forEach((property) => {
          if (property === 'team_inventory_id') {
            if (
              row[property]
                .toString()
                .toLowerCase()
  ...
  const handleSelectAllClick = (event?: ChangeEvent<HTMLInputElement>) => {
    if (event?.target.checked) {
      const newSelectedId = rows.map((row) => row.team_inventory_id)
  ...
  const deleteTeam = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/assigns/teams-inventories', { data: selected })
      .then((response) => {
        if (response.status === 204) {
          setData(
            data!.filter(
              (teamInventory) =>
                !selected.includes(teamInventory.team_inventory_id),
            ),
          )
          dispatch(
            openSnackbar({
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'inventories.placeholders.searchTeams',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                inventoryTeamsListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<TeamsInventoryDataProps>(
              rows,
              ...
              .map((row, index) => {
                ...
                const isItemSelected = isSelected(row.team_inventory_id)
                ...
                return (
                  ...
                  >
                    <TableCell
                      ...
                      sx={{ pl: 3 }}
                      onClick={(event) =>
                        handleClick(event, row.team_inventory_id)
                      }
                    >
                      <Checkbox
                        color="primary"
