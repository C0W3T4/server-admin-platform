...
import { inventoryGroupsListTableHeadCells } from '../../../../providers/InventoryProvider'
...
import { FormInventoryGroups } from '../../../forms/inventory/FormInventoryGroups'

interface InventoryGroupsListProps {
  inventoryData: InventoryDataProps | null
}

export const InventoryGroupsList = ({
  inventoryData,
}: InventoryGroupsListProps) => {
  ...
  const [orderBy, setOrderBy] = useState<string>('inventory_group_id')
  const [selected, setSelected] = useState<string[]>([])
  const [page, setPage] = useState<number>(0)
  const [rowsPerPage, setRowsPerPage] = useState<number>(5)
  const [search, setSearch] = useState<string>('')
  const [rows, setRows] = useState<InventoryGroupsDataProps[]>([])
  const [openDialog, setOpenDialog] = useState<boolean>(false)

  const { data, setData } = useAxiosGet<InventoryGroupsDataProps[]>(
    `api/assigns/inventories-groups/${id}/groups`,
  )
  ...
  const handleSearch = (
    ...
    if (newString) {
      const newRows = rows.filter((row: KeyedObject) => {
        ...
        const properties = ['inventory_group_id', 'name', 'description']
        let containsQuery = false

        properties.forEach((property) => {
          if (property === 'inventory_group_id') {
            if (
              row[property]
                .toString()
                .toLowerCase()
  ...
  const handleSelectAllClick = (event?: ChangeEvent<HTMLInputElement>) => {
    if (event?.target.checked) {
      const newSelectedId = rows.map((row) => row.inventory_group_id)
  ...
  const deleteGroup = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/assigns/inventories-groups', { data: selected })
      .then((response) => {
        if (response.status === 204) {
          setData(
            data!.filter(
              (inventoryGroup) =>
                !selected.includes(inventoryGroup.inventory_group_id),
            ),
          )
          dispatch(
            openSnackbar({
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'inventories.placeholders.searchGroups',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                inventoryGroupsListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<InventoryGroupsDataProps>(
              rows,
              ...
              .map((row, index) => {
                ...
                const isItemSelected = isSelected(row.inventory_group_id)
                ...
                return (
                  ...
                  >
                    <TableCell
                      ...
                      sx={{ pl: 3 }}
                      onClick={(event) =>
                        handleClick(event, row.inventory_group_id)
                      }
                    >
                      <Checkbox
                        color="primary"
                    ...
                    <TableCell align="left">
                      {row.group.description ? row.group.description : '---'}
                    </TableCell>
                  </TableRow>
                )
              })}
