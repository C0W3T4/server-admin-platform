...
import { inventoryUsersListTableHeadCells } from '../../../../providers/InventoryProvider'
import { userTypeMap } from '../../../../providers/userProvider'
import { ArrangementOrder, KeyedObject } from '../../../../types'
import { UsersInventoryDataProps } from '../../../../types/access'
...
import { FormInventoryUsers } from '../../../forms/inventory/FormInventoryUsers'

interface InventoryUsersListProps {
  inventoryData: InventoryDataProps | null
}

export const InventoryUsersList = ({
  inventoryData,
}: InventoryUsersListProps) => {
  ...
  const [orderBy, setOrderBy] = useState<string>('user_inventory_id')
  const [selected, setSelected] = useState<string[]>([])
  const [page, setPage] = useState<number>(0)
  const [rowsPerPage, setRowsPerPage] = useState<number>(5)
  const [search, setSearch] = useState<string>('')
  const [rows, setRows] = useState<UsersInventoryDataProps[]>([])
  const [openDialog, setOpenDialog] = useState<boolean>(false)

  const { data, setData } = useAxiosGet<UsersInventoryDataProps[]>(
    `api/assigns/users-inventories/${id}/users`,
  )
  ...
  const handleSearch = (
    ...
    if (newString) {
      const newRows = rows.filter((row: KeyedObject) => {
        ...
        const properties = ['user_inventory_id', 'username', 'user_type']
        let containsQuery = false

        properties.forEach((property) => {
          if (property === 'user_inventory_id') {
            if (
              row[property]
                .toString()
                .toLowerCase()
  ...
  const handleSelectAllClick = (event?: ChangeEvent<HTMLInputElement>) => {
    if (event?.target.checked) {
      const newSelectedId = rows.map((row) => row.user_inventory_id)
  ...
  const deleteUser = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/assigns/users-inventories', { data: selected })
      .then((response) => {
        if (response.status === 204) {
          setData(
            data!.filter(
              (userInventory) =>
                !selected.includes(userInventory.user_inventory_id),
            ),
          )
          dispatch(
            openSnackbar({
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'inventories.placeholders.searchUsers',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                inventoryUsersListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<UsersInventoryDataProps>(
              rows,
              ...
              .map((row, index) => {
                ...
                const isItemSelected = isSelected(row.user_inventory_id)
                ...
                return (
                  ...
                  >
                    <TableCell
                      ...
                      sx={{ pl: 3 }}
                      onClick={(event) =>
                        handleClick(event, row.user_inventory_id)
                      }
                    >
                      <Checkbox
                        color="primary"
