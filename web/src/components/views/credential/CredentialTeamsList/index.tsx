...
import { credentialTeamsListTableHeadCells } from '../../../../providers/CredentialProvider'
import { ArrangementOrder, KeyedObject } from '../../../../types'
import { TeamsCredentialDataProps } from '../../../../types/access'
...
import { FormCredentialTeams } from '../../../forms/credential/FormCredentialTeams'

interface CredentialTeamsListProps {
  credentialData?: CredentialDataProps | null
}

export const CredentialTeamsList = ({
  credentialData,
}: CredentialTeamsListProps) => {
  ...
  const [orderBy, setOrderBy] = useState<string>('team_credential_id')
  const [selected, setSelected] = useState<string[]>([])
  const [page, setPage] = useState<number>(0)
  const [rowsPerPage, setRowsPerPage] = useState<number>(5)
  const [search, setSearch] = useState<string>('')
  const [rows, setRows] = useState<TeamsCredentialDataProps[]>([])
  const [openDialog, setOpenDialog] = useState<boolean>(false)

  const { data, setData } = useAxiosGet<TeamsCredentialDataProps[]>(
    `api/assigns/teams-credentials/${id}/teams`,
  )
  ...
  const handleSearch = (
    ...
    if (newString) {
      const newRows = rows.filter((row: KeyedObject) => {
        ...
        const properties = ['team_credential_id', 'name', 'description']
        let containsQuery = false

        properties.forEach((property) => {
          if (property === 'team_credential_id') {
            if (
              row[property]
                .toString()
                .toLowerCase()
  ...
  const handleSelectAllClick = (event?: ChangeEvent<HTMLInputElement>) => {
    if (event?.target.checked) {
      const newSelectedId = rows.map((row) => row.team_credential_id)
  ...
  const deleteTeam = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/assigns/teams-credentials', { data: selected })
      .then((response) => {
        if (response.status === 204) {
          setData(
            data!.filter(
              (teamCredential) =>
                !selected.includes(teamCredential.team_credential_id),
            ),
          )
          dispatch(
            openSnackbar({
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'credentials.placeholders.searchTeams',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                credentialTeamsListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<TeamsCredentialDataProps>(
              rows,
              ...
              .map((row, index) => {
                ...
                const isItemSelected = isSelected(row.team_credential_id)
                ...
                return (
                  ...
                  >
                    <TableCell
                      ...
                      sx={{ pl: 3 }}
                      onClick={(event) =>
                        handleClick(event, row.team_credential_id)
                      }
                    >
                      <Checkbox
                        color="primary"
