...
import { templateTeamsListTableHeadCells } from '../../../../providers/TemplateProvider'
import { ArrangementOrder, KeyedObject } from '../../../../types'
import { TeamsTemplateDataProps } from '../../../../types/access'
...
import { FormTemplateTeams } from '../../../forms/template/FormTemplateTeams'

interface TemplateTeamsListProps {
  templateData?: TemplateDataProps | null
}

export const TemplateTeamsList = ({ templateData }: TemplateTeamsListProps) => {
  ...
  const [orderBy, setOrderBy] = useState<string>('team_template_id')
  const [selected, setSelected] = useState<string[]>([])
  const [page, setPage] = useState<number>(0)
  const [rowsPerPage, setRowsPerPage] = useState<number>(5)
  const [search, setSearch] = useState<string>('')
  const [rows, setRows] = useState<TeamsTemplateDataProps[]>([])
  const [openDialog, setOpenDialog] = useState<boolean>(false)

  const { data, setData } = useAxiosGet<TeamsTemplateDataProps[]>(
    `api/assigns/teams-templates/${id}/teams`,
  )
  ...
  const handleSearch = (
    ...
    if (newString) {
      const newRows = rows.filter((row: KeyedObject) => {
        ...
        const properties = ['team_template_id', 'name', 'description']
        let containsQuery = false

        properties.forEach((property) => {
          if (property === 'team_template_id') {
            if (
              row[property]
                .toString()
                .toLowerCase()
  ...
  const handleSelectAllClick = (event?: ChangeEvent<HTMLInputElement>) => {
    if (event?.target.checked) {
      const newSelectedId = rows.map((row) => row.team_template_id)
  ...
  const deleteTeam = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/assigns/teams-templates', { data: selected })
      .then((response) => {
        if (response.status === 204) {
          setData(
            data!.filter(
              (teamTemplate) =>
                !selected.includes(teamTemplate.team_template_id),
            ),
          )
          dispatch(
            openSnackbar({
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'templates.placeholders.searchTeams',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                templateTeamsListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<TeamsTemplateDataProps>(
              rows,
              ...
              .map((row, index) => {
                ...
                const isItemSelected = isSelected(row.team_template_id)
                ...
                return (
                  ...
                  >
                    <TableCell
                      ...
                      sx={{ pl: 3 }}
                      onClick={(event) =>
                        handleClick(event, row.team_template_id)
                      }
                    >
                      <Checkbox
                        color="primary"
