...
import { hostTeamsListTableHeadCells } from '../../../../providers/HostProvider'
import { ArrangementOrder, KeyedObject } from '../../../../types'
import { TeamsHostDataProps } from '../../../../types/access'
...
import { FormHostTeams } from '../../../forms/host/FormHostTeams'

interface HostTeamsListProps {
  hostData?: HostDataProps | null
}

export const HostTeamsList = ({ hostData }: HostTeamsListProps) => {
  ...
  const [orderBy, setOrderBy] = useState<string>('team_host_id')
  const [selected, setSelected] = useState<string[]>([])
  const [page, setPage] = useState<number>(0)
  const [rowsPerPage, setRowsPerPage] = useState<number>(5)
  const [search, setSearch] = useState<string>('')
  const [rows, setRows] = useState<TeamsHostDataProps[]>([])
  const [openDialog, setOpenDialog] = useState<boolean>(false)

  const { data, setData } = useAxiosGet<TeamsHostDataProps[]>(
    `api/assigns/teams-hosts/${id}/teams`,
  )
  ...
  const handleSearch = (
    ...
    if (newString) {
      const newRows = rows.filter((row: KeyedObject) => {
        ...
        const properties = ['team_host_id', 'name', 'description']
        let containsQuery = false

        properties.forEach((property) => {
          if (property === 'team_host_id') {
            if (
              row[property]
                .toString()
                .toLowerCase()
  ...
  const handleSelectAllClick = (event?: ChangeEvent<HTMLInputElement>) => {
    if (event?.target.checked) {
      const newSelectedId = rows.map((row) => row.team_host_id)
  ...
  const deleteTeam = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/assigns/teams-hosts', { data: selected })
      .then((response) => {
        if (response.status === 204) {
          setData(
            data!.filter(
              (teamHost) => !selected.includes(teamHost.team_host_id),
            ),
          )
          dispatch(
            openSnackbar({
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'hosts.placeholders.searchTeams',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                hostTeamsListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<TeamsHostDataProps>(rows, getComparator(order, orderBy))
              .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
              .map((row, index) => {
                /** Make sure no display bugs if row isn't an OrderData object */
                if (typeof row === 'number') return null

                const isItemSelected = isSelected(row.team_host_id)
                ...
                return (
                  ...
                  >
                    <TableCell
                      padding="checkbox"
                      sx={{ pl: 3 }}
                      onClick={(event) => handleClick(event, row.team_host_id)}
                    >
                      <Checkbox
                        color="primary"
