...
import { hostUsersListTableHeadCells } from '../../../../providers/HostProvider'
import { userTypeMap } from '../../../../providers/userProvider'
import { ArrangementOrder, KeyedObject } from '../../../../types'
import { UsersHostDataProps } from '../../../../types/access'
...
import { FormHostUsers } from '../../../forms/host/FormHostUsers'

interface HostUsersListProps {
  hostData: HostDataProps | null
}

export const HostUsersList = ({ hostData }: HostUsersListProps) => {
  ...
  const [orderBy, setOrderBy] = useState<string>('user_host_id')
  const [selected, setSelected] = useState<string[]>([])
  const [page, setPage] = useState<number>(0)
  const [rowsPerPage, setRowsPerPage] = useState<number>(5)
  const [search, setSearch] = useState<string>('')
  const [rows, setRows] = useState<UsersHostDataProps[]>([])
  const [openDialog, setOpenDialog] = useState<boolean>(false)

  const { data, setData } = useAxiosGet<UsersHostDataProps[]>(
    `api/assigns/users-hosts/${id}/users`,
  )
  ...
  const handleSearch = (
    ...
    if (newString) {
      const newRows = rows.filter((row: KeyedObject) => {
        ...
        const properties = ['user_host_id', 'username', 'user_type']
        let containsQuery = false

        properties.forEach((property) => {
          if (property === 'user_host_id') {
            if (
              row[property]
                .toString()
                .toLowerCase()
  ...
  const handleSelectAllClick = (event?: ChangeEvent<HTMLInputElement>) => {
    if (event?.target.checked) {
      const newSelectedId = rows.map((row) => row.user_host_id)
  ...
  const deleteUser = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/assigns/users-hosts', { data: selected })
      .then((response) => {
        if (response.status === 204) {
          setData(
            data!.filter(
              (userHost) => !selected.includes(userHost.user_host_id),
            ),
          )
          dispatch(
            openSnackbar({
  ...
  return (
    <MainCard
      title={`${hostData!.hostname} -> ${formatMessage({
        id: 'hosts.labels.users',
      })}`}
      content={false}
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'hosts.placeholders.searchUsers',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                hostUsersListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<UsersHostDataProps>(rows, getComparator(order, orderBy))
              .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
              .map((row, index) => {
                /** Make sure no display bugs if row isn't an OrderData object */
                if (typeof row === 'number') return null

                const isItemSelected = isSelected(row.user_host_id)
                ...
                return (
                  ...
                  >
                    <TableCell
                      padding="checkbox"
                      sx={{ pl: 3 }}
                      onClick={(event) => handleClick(event, row.user_host_id)}
                    >
                      <Checkbox
                        color="primary"
