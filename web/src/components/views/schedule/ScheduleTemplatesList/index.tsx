...
import { scheduleTemplatesListTableHeadCells } from '../../../../providers/ScheduleProvider'
import { ArrangementOrder, KeyedObject } from '../../../../types'
import { ScheduleDataProps } from '../../../../types/schedule'
import { TemplatesScheduleDataProps } from '../../../../types/template'
...
interface ScheduleTemplatesListProps {
  scheduleData: ScheduleDataProps | null
}
...
export const ScheduleTemplatesList = ({
  ...
  const [rows, setRows] = useState<TemplatesScheduleDataProps[]>([])

  const { data, setData } = useAxiosGet<TemplatesScheduleDataProps[]>(
    `api/assigns/templates-schedules/${id}/templates`,
  )
  ...
  const handleSearch = (
    ...
    if (newString) {
      const newRows = rows.filter((row: KeyedObject) => {
        ...
        const properties = ['template_schedule_id', 'name', 'description']
        let containsQuery = false

        properties.forEach((property) => {
          if (property === 'template_schedule_id') {
            if (
              row[property]
                .toString()
                .toLowerCase()
      ...
      .then((response) => {
        if (response.status === 204) {
          setData(
            data!.filter(
              (scheduleTemplate) =>
                !selected.includes(scheduleTemplate.template_schedule_id),
            ),
          )
          dispatch(
            openSnackbar({
  ...
  return (
    <MainCard
      title={`${scheduleData!.name} -> ${formatMessage({
        id: 'schedules.labels.templates',
      })}`}
      content={false}
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'schedules.placeholders.searchTemplates',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                scheduleTemplatesListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<TemplatesScheduleDataProps>(
              rows,
