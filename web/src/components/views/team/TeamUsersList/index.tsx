...
import { teamUsersListTableHeadCells } from '../../../../providers/TeamProvider'
import { userTypeMap } from '../../../../providers/userProvider'
import { ArrangementOrder, KeyedObject } from '../../../../types'
import { UsersTeamDataProps } from '../../../../types/access'
...
import { FormTeamUsers } from '../../../forms/team/FormTeamUsers'

interface TeamUsersListProps {
  teamData: TeamDataProps | null
}

export const TeamUsersList = ({ teamData }: TeamUsersListProps) => {
  ...
  const [orderBy, setOrderBy] = useState<string>('user_team_id')
  const [selected, setSelected] = useState<string[]>([])
  const [page, setPage] = useState<number>(0)
  const [rowsPerPage, setRowsPerPage] = useState<number>(5)
  const [search, setSearch] = useState<string>('')
  const [rows, setRows] = useState<UsersTeamDataProps[]>([])
  const [openDialog, setOpenDialog] = useState<boolean>(false)

  const { data, setData } = useAxiosGet<UsersTeamDataProps[]>(
    `api/assigns/users-teams/${id}/users`,
  )
  ...
  const handleSearch = (
    ...
    if (newString) {
      const newRows = rows.filter((row: KeyedObject) => {
        ...
        const properties = ['user_team_id', 'username', 'user_type']
        let containsQuery = false

        properties.forEach((property) => {
          if (property === 'user_team_id') {
            if (
              row[property]
                .toString()
                .toLowerCase()
  ...
  const handleSelectAllClick = (event?: ChangeEvent<HTMLInputElement>) => {
    if (event?.target.checked) {
      const newSelectedId = rows.map((row) => row.user_team_id)
  ...
  const deleteUser = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/assigns/users-teams', { data: selected })
      .then((response) => {
        if (response.status === 204) {
          setData(
            data!.filter(
              (userTeam) => !selected.includes(userTeam.user_team_id),
            ),
          )
          dispatch(
            openSnackbar({
  ...
  return (
    <MainCard
      title={`${teamData!.name} -> ${formatMessage({
        id: 'teams.labels.users',
      })}`}
      content={false}
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'teams.placeholders.searchUsers',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                teamUsersListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<UsersTeamDataProps>(rows, getComparator(order, orderBy))
              .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
              .map((row, index) => {
                /** Make sure no display bugs if row isn't an OrderData object */
                if (typeof row === 'number') return null

                const isItemSelected = isSelected(row.user_team_id)
                ...
                return (
                  ...
                  >
                    <TableCell
                      padding="checkbox"
                      sx={{ pl: 3 }}
                      onClick={(event) => handleClick(event, row.user_team_id)}
                    >
                      <Checkbox
                        color="primary"
