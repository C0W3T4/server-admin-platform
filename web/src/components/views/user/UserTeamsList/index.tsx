...
import { userTeamsListTableHeadCells } from '../../../../providers/userProvider'
import { ArrangementOrder, KeyedObject } from '../../../../types'
import { UserTeamsDataProps } from '../../../../types/access'
...
export const UserTeamsList = ({ userData }: UserTeamsListProps) => {
  ...
  const [rows, setRows] = useState<UserTeamsDataProps[]>([])

  const { data, setData } = useAxiosGet<UserTeamsDataProps[]>(
    `api/assigns/users-teams/${id}/teams`,
  )
  ...
  const handleSearch = (
    ...
    if (newString) {
      const newRows = rows.filter((row: KeyedObject) => {
        ...
        const properties = ['user_team_id', 'name', 'description']
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'users.placeholders.searchTeams',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                userTeamsListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<UserTeamsDataProps>(rows, getComparator(order, orderBy))
