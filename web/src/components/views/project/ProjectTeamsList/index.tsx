...
import { projectTeamsListTableHeadCells } from '../../../../providers/ProjectProvider'
import { ArrangementOrder, KeyedObject } from '../../../../types'
import { TeamsProjectDataProps } from '../../../../types/access'
...
import { FormProjectTeams } from '../../../forms/project/FormProjectTeams'

interface ProjectTeamsListProps {
  projectData?: ProjectDataProps | null
}

export const ProjectTeamsList = ({ projectData }: ProjectTeamsListProps) => {
  ...
  const [orderBy, setOrderBy] = useState<string>('team_project_id')
  const [selected, setSelected] = useState<string[]>([])
  const [page, setPage] = useState<number>(0)
  const [rowsPerPage, setRowsPerPage] = useState<number>(5)
  const [search, setSearch] = useState<string>('')
  const [rows, setRows] = useState<TeamsProjectDataProps[]>([])
  const [openDialog, setOpenDialog] = useState<boolean>(false)

  const { data, setData } = useAxiosGet<TeamsProjectDataProps[]>(
    `api/assigns/teams-projects/${id}/teams`,
  )
  ...
  const handleSearch = (
    ...
    if (newString) {
      const newRows = rows.filter((row: KeyedObject) => {
        ...
        const properties = ['team_project_id', 'name', 'description']
        let containsQuery = false

        properties.forEach((property) => {
          if (property === 'team_project_id') {
            if (
              row[property]
                .toString()
                .toLowerCase()
  ...
  const handleSelectAllClick = (event?: ChangeEvent<HTMLInputElement>) => {
    if (event?.target.checked) {
      const newSelectedId = rows.map((row) => row.team_project_id)
  ...
  const deleteTeam = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/assigns/teams-projects', { data: selected })
      .then((response) => {
        if (response.status === 204) {
          setData(
            data!.filter(
              (teamProject) => !selected.includes(teamProject.team_project_id),
            ),
          )
          dispatch(
            openSnackbar({
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'projects.placeholders.searchTeams',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                projectTeamsListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<TeamsProjectDataProps>(
              rows,
              ...
              .map((row, index) => {
                ...
                const isItemSelected = isSelected(row.team_project_id)
                ...
                return (
                  ...
                  >
                    <TableCell
                      ...
                      sx={{ pl: 3 }}
                      onClick={(event) =>
                        handleClick(event, row.team_project_id)
                      }
                    >
                      <Checkbox
                        color="primary"
