...
import { FormProjectSchedules } from '../../../forms/project/FormProjectSchedules'
...
interface ProjectSchedulesListProps {
  projectData: ProjectDataProps | null
}

export const ProjectSchedulesList = ({
  projectData,
}: ProjectSchedulesListProps) => {
  ...
  const [orderBy, setOrderBy] = useState<string>('project_schedule_id')
  const [selected, setSelected] = useState<string[]>([])
  const [page, setPage] = useState<number>(0)
  const [rowsPerPage, setRowsPerPage] = useState<number>(5)
  const [search, setSearch] = useState<string>('')
  const [rows, setRows] = useState<ProjectSchedulesDataProps[]>([])
  const [openDialog, setOpenDialog] = useState<boolean>(false)
  const [scheduleInfoData, setScheduleInfoData] = useState<
    JobsScheduleInfoDataProps[]
  >([])

  const { data, setData } = useAxiosGet<ProjectSchedulesDataProps[]>(
    `api/assigns/projects-schedules/${id}/schedules`,
  )
    ...
    if (newString) {
      const newRows = rows.filter((row: KeyedObject) => {
        ...
        properties.forEach((property) => {
          if (property === 'project_schedule_id' || property === 'next_date') {
            if (
              row[property]
                .toString()
                .toLowerCase()
  ...
  const handleSelectAllClick = (event?: ChangeEvent<HTMLInputElement>) => {
    if (event?.target.checked) {
      const newSelectedId = rows.map((row) => row.project_schedule_id)
  ...
  const deleteSchedule = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/assigns/projects-schedules', { data: selected })
      .then((response) => {
        if (response.status === 204) {
          setData(
            data!.filter(
              (projectSchedule) =>
                !selected.includes(projectSchedule.project_schedule_id),
            ),
          )
          dispatch(
            openSnackbar({
  ...
  const getScheduleInfo = async () => {
    await api
      .get(`api/assigns/projects-schedules/${id}/schedules/info`)
  ...
  console.log(data)
  console.log(scheduleInfoData)

  return (
    <MainCard
      title={`${projectData!.name} -> ${formatMessage({
        id: 'projects.labels.schedules',
      })}`}
      content={false}
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'projects.placeholders.searchSchedules',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                projectSchedulesListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<ProjectSchedulesDataProps>(
              rows,
              ...
              .map((row, index) => {
                ...
                const isItemSelected = isSelected(row.project_schedule_id)
                ...
                return (
                  ...
                  >
                    <TableCell
                      ...
                      sx={{ pl: 3 }}
                      onClick={(event) =>
                        handleClick(event, row.project_schedule_id)
                      }
                    >
                      <Checkbox
                        color="primary"
