...
import { groupHostsListTableHeadCells } from '../../../../providers/GroupProvider'
...
import { FormGroupHosts } from '../../../forms/group/FormGroupHosts'

interface GroupHostsListProps {
  groupData: GroupDataProps | null
}

export const GroupHostsList = ({ groupData }: GroupHostsListProps) => {
  ...
  const [orderBy, setOrderBy] = useState<string>('group_host_id')
  const [selected, setSelected] = useState<string[]>([])
  const [page, setPage] = useState<number>(0)
  const [rowsPerPage, setRowsPerPage] = useState<number>(5)
  const [search, setSearch] = useState<string>('')
  const [rows, setRows] = useState<GroupHostsDataProps[]>([])
  const [openDialog, setOpenDialog] = useState<boolean>(false)

  const { data, setData } = useAxiosGet<GroupHostsDataProps[]>(
    `api/assigns/groups-hosts/${id}/hosts`,
  )
  ...
  const handleSearch = (
    ...
    if (newString) {
      const newRows = rows.filter((row: KeyedObject) => {
        ...
        const properties = ['group_host_id', 'hostname', 'ipv4', 'host_status']
        let containsQuery = false

        properties.forEach((property) => {
          if (property === 'group_host_id') {
            if (
              row[property]
                .toString()
                .toLowerCase()
  ...
  const handleSelectAllClick = (event?: ChangeEvent<HTMLInputElement>) => {
    if (event?.target.checked) {
      const newSelectedId = rows.map((row) => row.group_host_id)
  ...
  const deleteHost = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/assigns/groups-hosts', { data: selected })
      .then((response) => {
        if (response.status === 204) {
          setData(
            data!.filter(
              (groupHost) => !selected.includes(groupHost.group_host_id),
            ),
          )
          dispatch(
            openSnackbar({
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'groups.placeholders.searchHosts',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                groupHostsListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<GroupHostsDataProps>(
              rows,
              ...
              .map((row, index) => {
                ...
                const isItemSelected = isSelected(row.group_host_id)
                ...
                return (
                  ...
                  >
                    <TableCell
                      padding="checkbox"
                      sx={{ pl: 3 }}
                      onClick={(event) => handleClick(event, row.group_host_id)}
                    >
                      <Checkbox
                        color="primary"
                    ...
                    <TableCell align="left">{row.host.hostname}</TableCell>
                    <TableCell align="left">{row.host.ipv4}</TableCell>
                    <TableCell align="left">
                      {hostStatusMap.get(row.host.host_status)?.icon}
                      <FormattedMessage
                        id={hostStatusMap.get(row.host.host_status)?.label}
                      />
                    </TableCell>
                  </TableRow>
                )
