...
import { groupUsersListTableHeadCells } from '../../../../providers/GroupProvider'
import { userTypeMap } from '../../../../providers/userProvider'
import { ArrangementOrder, KeyedObject } from '../../../../types'
import { UsersGroupDataProps } from '../../../../types/access'
...
import { FormGroupUsers } from '../../../forms/group/FormGroupUsers'

interface GroupUsersListProps {
  groupData: GroupDataProps | null
}

export const GroupUsersList = ({ groupData }: GroupUsersListProps) => {
  ...
  const [orderBy, setOrderBy] = useState<string>('user_group_id')
  const [selected, setSelected] = useState<string[]>([])
  const [page, setPage] = useState<number>(0)
  const [rowsPerPage, setRowsPerPage] = useState<number>(5)
  const [search, setSearch] = useState<string>('')
  const [rows, setRows] = useState<UsersGroupDataProps[]>([])
  const [openDialog, setOpenDialog] = useState<boolean>(false)

  const { data, setData } = useAxiosGet<UsersGroupDataProps[]>(
    `api/assigns/users-groups/${id}/users`,
  )
  ...
  const handleSearch = (
    ...
    if (newString) {
      const newRows = rows.filter((row: KeyedObject) => {
        ...
        const properties = ['user_group_id', 'username', 'user_type']
  ...
  const handleSelectAllClick = (event?: ChangeEvent<HTMLInputElement>) => {
    if (event?.target.checked) {
      const newSelectedId = rows.map((row) => row.user_group_id)
  ...
  const deleteUser = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/assigns/users-groups', { data: selected })
      .then((response) => {
        if (response.status === 204) {
          setData(
            data!.filter(
              (userGroup) => !selected.includes(userGroup.user_group_id),
            ),
          )
          dispatch(
            openSnackbar({
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'groups.placeholders.searchUsers',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                groupUsersListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<UsersGroupDataProps>(
              rows,
              ...
              .map((row, index) => {
                ...
                const isItemSelected = isSelected(row.user_group_id)
                ...
                return (
                  ...
                  >
                    <TableCell
                      padding="checkbox"
                      sx={{ pl: 3 }}
                      onClick={(event) => handleClick(event, row.user_group_id)}
                    >
                      <Checkbox
                        color="primary"
