...
import { organizationTeamsListTableHeadCells } from '../../../../providers/OrganizationProvider'
import { ArrangementOrder, KeyedObject } from '../../../../types'
import { TeamsOrganizationDataProps } from '../../../../types/access'
...
import { FormOrganizationTeams } from '../../../forms/organization/FormOrganizationTeams'

interface OrganizationTeamsListProps {
  organizationData?: OrganizationDataProps | null
}
...
export const OrganizationTeamsList = ({
  ...
  const [orderBy, setOrderBy] = useState<string>('team_organization_id')
  const [selected, setSelected] = useState<string[]>([])
  const [page, setPage] = useState<number>(0)
  const [rowsPerPage, setRowsPerPage] = useState<number>(5)
  const [search, setSearch] = useState<string>('')
  const [rows, setRows] = useState<TeamsOrganizationDataProps[]>([])
  const [openDialog, setOpenDialog] = useState<boolean>(false)

  const { data, setData } = useAxiosGet<TeamsOrganizationDataProps[]>(
    `api/assigns/teams-organizations/${id}/teams`,
  )
  ...
  const handleSearch = (
    ...
    if (newString) {
      const newRows = rows.filter((row: KeyedObject) => {
        ...
        const properties = ['team_organization_id', 'name', 'description']
        let containsQuery = false

        properties.forEach((property) => {
          if (property === 'team_organization_id') {
            if (
              row[property]
                .toString()
                .toLowerCase()
  ...
  const handleSelectAllClick = (event?: ChangeEvent<HTMLInputElement>) => {
    if (event?.target.checked) {
      const newSelectedId = rows.map((row) => row.team_organization_id)
  ...
  const deleteTeam = async (selected: string[]) => {
    showLoading()
    await api
      .delete('api/assigns/teams-organizations', { data: selected })
      .then((response) => {
        if (response.status === 204) {
          setData(
            data!.filter(
              (teamOrganization) =>
                !selected.includes(teamOrganization.team_organization_id),
            ),
          )
          dispatch(
            openSnackbar({
  ...
  return (
    <MainCard
      title={`${organizationData!.name} -> ${formatMessage({
        id: 'organizations.labels.teams',
      })}`}
      content={false}
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'organizations.placeholders.searchTeams',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                organizationTeamsListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<TeamsOrganizationDataProps>(
              rows,
              ...
              .map((row, index) => {
                ...
                const isItemSelected = isSelected(row.team_organization_id)
                ...
                return (
                  ...
                  >
                    <TableCell
                      ...
                      sx={{ pl: 3 }}
                      onClick={(event) =>
                        handleClick(event, row.team_organization_id)
                      }
                    >
                      <Checkbox
                        color="primary"
