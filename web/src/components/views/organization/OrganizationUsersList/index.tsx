...
import { organizationUsersListTableHeadCells } from '../../../../providers/OrganizationProvider'
...
import { FormOrganizationUsers } from '../../../forms/organization/FormOrganizationUsers'
...
export const OrganizationUsersList = ({
  ...
  const [rows, setRows] = useState<UsersOrganizationDataProps[]>([])
  const [openDialog, setOpenDialog] = useState<boolean>(false)

  const { data, setData } = useAxiosGet<UsersOrganizationDataProps[]>(
    `api/assigns/users-organizations/${id}/users`,
  )
  ...
  const handleSearch = (
    ...
    if (newString) {
      const newRows = rows.filter((row: KeyedObject) => {
        ...
        const properties = ['user_organization_id', 'username', 'user_type']
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              placeholder={formatMessage({
                id: 'organizations.placeholders.searchUsers',
              })}
              value={search}
              size="small"
            />
        ...
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
          ...
          <TableHead>
            <TableRow>
              ...
              {selected.length <= 0 &&
                organizationUsersListTableHeadCells.map((headCell) => (
                  <TableCell
                    key={headCell.id}
          ...
          {/* table body */}
          <TableBody>
            {stableSort<UsersOrganizationDataProps>(
              rows,
