...
export const FormOrganizationTeams = ({
  open,
  handleCloseDialog,
  data: organizationData,
  setNewData,
  oldData,
}: DialogFormProps<OrganizationDataProps, TeamsOrganizationDataProps[]>) => {
  const theme = useTheme()
  const { id } = useParams()
  const { formatMessage } = useIntl()
  const { showLoading, hideLoading } = useLoading()

  const { data } = useAxiosGet<TeamDataProps[]>('api/teams')
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (formData) => {
    ...
    await api
      .request({
        method: 'POST',
        url: '/api/assigns/teams-organizations',
        data: formData,
      })
      .then((response) => {
        if (setNewData && oldData) {
          setNewData([
            ...oldData,
            {
              team_organization_id: response.data.team_organization_id,
      ...
      {open && (
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogTitle>{`${formatMessage({
            id: 'organizations.teams.dialog.title',
                ...
                <Grid container spacing={1}>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <Typography variant="subtitle1" align="left">
                      <FormattedMessage id="organizations.labels.teams" />
                    </Typography>
                  </Grid>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <FormControl
