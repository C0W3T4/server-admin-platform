...
export const FormGroupTeams = ({
  open,
  handleCloseDialog,
  data: groupData,
  setNewData,
  oldData,
}: DialogFormProps<GroupDataProps, TeamsGroupDataProps[]>) => {
  ...
  const { data } = useAxiosGet<TeamsOrganizationDataProps[]>(
    `api/assigns/teams-organizations/${groupData?.organization.id}/teams`,
  )
      ...
      .then((response) => {
        if (setNewData && oldData) {
          setNewData([
            ...oldData,
            {
              team_group_id: response.data.team_group_id,
                ...
                <Grid container spacing={1}>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <Typography variant="subtitle1" align="left">
                      <FormattedMessage id="groups.labels.teams" />
                    </Typography>
                  </Grid>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <FormControl
