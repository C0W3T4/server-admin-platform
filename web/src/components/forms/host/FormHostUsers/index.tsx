...
export const FormHostUsers = ({
  open,
  handleCloseDialog,
  data: hostData,
  setNewData,
  oldData,
}: DialogFormProps<HostDataProps, UsersHostDataProps[]>) => {
  ...
  const { data } = useAxiosGet<UsersOrganizationDataProps[]>(
    `api/assigns/users-organizations/${hostData?.organization.id}/users`,
  )
      ...
      .then((response) => {
        if (setNewData && oldData) {
          setNewData([
            ...oldData,
            {
              user_host_id: response.data.user_host_id,
                ...
                <Grid container spacing={1}>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <Typography variant="subtitle1" align="left">
                      <FormattedMessage id="hosts.labels.users" />
                    </Typography>
                  </Grid>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <FormControl
