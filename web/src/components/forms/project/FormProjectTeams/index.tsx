...
export const FormProjectTeams = ({
  open,
  handleCloseDialog,
  data: projectData,
  setNewData,
  oldData,
}: DialogFormProps<ProjectDataProps, TeamsProjectDataProps[]>) => {
  ...
  const { data } = useAxiosGet<TeamsOrganizationDataProps[]>(
    `api/assigns/teams-organizations/${projectData?.organization.id}/teams`,
  )
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (formData) => {
    ...
    await api
      .request({
        method: 'POST',
        url: '/api/assigns/teams-projects',
        data: formData,
      })
      .then((response) => {
        if (setNewData && oldData) {
          setNewData([
            ...oldData,
            {
              team_project_id: response.data.team_project_id,
      ...
      {open && (
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogTitle>{`${formatMessage({
            id: 'projects.teams.dialog.title',
                ...
                <Grid container spacing={1}>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <Typography variant="subtitle1" align="left">
                      <FormattedMessage id="projects.labels.teams" />
                    </Typography>
                  </Grid>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <FormControl
