...
export const FormProjectUsers = ({
  open,
  handleCloseDialog,
  data: projectData,
  setNewData,
  oldData,
}: DialogFormProps<ProjectDataProps, UsersProjectDataProps[]>) => {
  ...
  const { data } = useAxiosGet<UsersOrganizationDataProps[]>(
    `api/assigns/users-organizations/${projectData?.organization.id}/users`,
  )
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (formData) => {
    ...
    await api
      .request({
        method: 'POST',
        url: '/api/assigns/users-projects',
        data: formData,
      })
      .then((response) => {
        if (setNewData && oldData) {
          setNewData([
            ...oldData,
            {
              user_project_id: response.data.user_project_id,
      ...
      {open && (
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogTitle>{`${formatMessage({
            id: 'projects.users.dialog.title',
          })} -> ${projectData!.name}`}</DialogTitle>
          <DialogContent>
                ...
                <Grid container spacing={1}>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <Typography variant="subtitle1" align="left">
                      <FormattedMessage id="projects.labels.users" />
                    </Typography>
                  </Grid>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <FormControl
