...
export const FormProjectSchedules = ({
  open,
  handleCloseDialog,
  data: projectData,
  setNewData,
  oldData,
}: DialogFormProps<ProjectDataProps, ProjectSchedulesDataProps[]>) => {
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (formData) => {
    ...
    await api
      .request({
        method: 'POST',
        url: '/api/assigns/projects-schedules',
        data: formData,
      })
      .then((response) => {
      ...
      {open && (
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogTitle>{`${formatMessage({
            id: 'projects.schedules.dialog.title',
                ...
                <Grid container spacing={1}>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <Typography variant="subtitle1" align="left">
                      <FormattedMessage id="projects.labels.schedules" />
                    </Typography>
                  </Grid>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <FormControl
                      ...
                      >
                        {data &&
                          data
                            .filter(
                              (schedule) =>
                                schedule.schedule_type === ScheduleType.PROJECT,
                            )
                            .map((schedule) => (
