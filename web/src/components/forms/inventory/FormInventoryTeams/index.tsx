...
export const FormInventoryTeams = ({
  open,
  handleCloseDialog,
  data: inventoryData,
  setNewData,
  oldData,
}: DialogFormProps<InventoryDataProps, TeamsInventoryDataProps[]>) => {
  ...
  const { data } = useAxiosGet<TeamsOrganizationDataProps[]>(
    `api/assigns/teams-organizations/${inventoryData?.organization.id}/teams`,
  )
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (formData) => {
    ...
    await api
      .request({
        method: 'POST',
        url: '/api/assigns/teams-inventories',
        data: formData,
      })
      .then((response) => {
        if (setNewData && oldData) {
          setNewData([
            ...oldData,
            {
              team_inventory_id: response.data.team_inventory_id,
      ...
      {open && (
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogTitle>{`${formatMessage({
            id: 'inventories.teams.dialog.title',
                ...
                <Grid container spacing={1}>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <Typography variant="subtitle1" align="left">
                      <FormattedMessage id="inventories.labels.teams" />
                    </Typography>
                  </Grid>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <FormControl
