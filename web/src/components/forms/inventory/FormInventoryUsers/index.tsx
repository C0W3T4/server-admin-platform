...
export const FormInventoryUsers = ({
  open,
  handleCloseDialog,
  data: inventoryData,
  setNewData,
  oldData,
}: DialogFormProps<InventoryDataProps, UsersInventoryDataProps[]>) => {
  ...
  const { data } = useAxiosGet<UsersOrganizationDataProps[]>(
    `api/assigns/users-organizations/${inventoryData?.organization.id}/users`,
  )
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (formData) => {
    ...
    await api
      .request({
        method: 'POST',
        url: '/api/assigns/users-inventories',
        data: formData,
      })
      .then((response) => {
        if (setNewData && oldData) {
          setNewData([
            ...oldData,
            {
              user_inventory_id: response.data.user_inventory_id,
      ...
      {open && (
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogTitle>{`${formatMessage({
            id: 'inventories.users.dialog.title',
                ...
                <Grid container spacing={1}>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <Typography variant="subtitle1" align="left">
                      <FormattedMessage id="inventories.labels.users" />
                    </Typography>
                  </Grid>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <FormControl
