...
export const FormTeamUsers = ({
  open,
  handleCloseDialog,
  data: teamData,
  setNewData,
  oldData,
}: DialogFormProps<TeamDataProps, UsersTeamDataProps[]>) => {
      ...
      .then((response) => {
        if (setNewData && oldData) {
          setNewData([
            ...oldData,
            {
              user_team_id: response.data.user_team_id,
      ...
      {open && (
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogTitle>{`${formatMessage({
            id: 'teams.users.dialog.title',
          })} -> ${teamData!.name}`}</DialogTitle>
          <DialogContent>
                ...
                <Grid container spacing={1}>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <Typography variant="subtitle1" align="left">
                      <FormattedMessage id="teams.labels.users" />
                    </Typography>
                  </Grid>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <FormControl
