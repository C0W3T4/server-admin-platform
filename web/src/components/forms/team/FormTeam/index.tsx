...
export const FormTeam = ({ defaultValues, mode }: FormProps<TeamDataProps>) => {
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (formData) => {
    ...
    await api
      .request({
        ...
        url: mode === 'create' ? '/api/teams' : `/api/teams/${id}`,
        data: formData,
      })
      .then((response) => {
        dispatch(
      ...
      <SubCard
        title={
          mode === 'edit' ? (
            ...
            <FormattedMessage id="teams.labels.new" />
          )
        }
        ...
        <Grid container spacing={2}>
          <Grid item xs={12} sm={12} md={6} lg={6} xl={6}>
            <TextField
              fullWidth
              id="name"
              label={<FormattedMessage id="teams.labels.name" />}
          ...
          <Grid item xs={12} sm={12} md={6} lg={6} xl={6}>
            <TextField
              ...
              multiline
              rows={1}
              label={<FormattedMessage id="teams.labels.description" />}
