...
export const FormCredentialTeams = ({
  open,
  handleCloseDialog,
  data: credentialData,
  setNewData,
  oldData,
}: DialogFormProps<CredentialDataProps, TeamsCredentialDataProps[]>) => {
  ...
  const { data } = useAxiosGet<TeamsOrganizationDataProps[]>(
    `api/assigns/teams-organizations/${credentialData?.organization.id}/teams`,
  )
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (formData) => {
    ...
    await api
      .request({
        method: 'POST',
        url: '/api/assigns/teams-credentials',
        data: formData,
      })
      .then((response) => {
        setNewData &&
          ...
          setNewData([
            ...oldData,
            {
              team_credential_id: response.data.team_credential_id,
      ...
      {open && (
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogTitle>{`${formatMessage({
            id: 'credentials.teams.dialog.title',
                ...
                <Grid container spacing={1}>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <Typography variant="subtitle1" align="left">
                      <FormattedMessage id="credentials.labels.teams" />
                    </Typography>
                  </Grid>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <FormControl
