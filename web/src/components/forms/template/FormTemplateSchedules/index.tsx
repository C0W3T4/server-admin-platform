...
export const FormTemplateSchedules = ({
  open,
  handleCloseDialog,
  data: templateData,
  setNewData,
  oldData,
}: DialogFormProps<TemplateDataProps, TemplateSchedulesDataProps[]>) => {
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (formData) => {
    ...
    await api
      .request({
        method: 'POST',
        url: '/api/assigns/templates-schedules',
        data: formData,
      })
      .then((response) => {
      ...
      {open && (
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogTitle>{`${formatMessage({
            id: 'templates.schedules.dialog.title',
                ...
                <Grid container spacing={1}>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <Typography variant="subtitle1" align="left">
                      <FormattedMessage id="templates.labels.schedules" />
                    </Typography>
                  </Grid>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <FormControl
