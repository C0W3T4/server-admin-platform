...
export const FormTemplateTeams = ({
  open,
  handleCloseDialog,
  data: templateData,
  setNewData,
  oldData,
}: DialogFormProps<TemplateDataProps, TeamsTemplateDataProps[]>) => {
  ...
  const { data } = useAxiosGet<TeamsOrganizationDataProps[]>(
    `api/assigns/teams-organizations/${templateData?.organization.id}/teams`,
  )
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (formData) => {
    ...
    await api
      .request({
        method: 'POST',
        url: '/api/assigns/teams-templates',
        data: formData,
      })
      .then((response) => {
        if (setNewData && oldData) {
          setNewData([
            ...oldData,
            {
              team_template_id: response.data.team_template_id,
      ...
      {open && (
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogTitle>{`${formatMessage({
            id: 'templates.teams.dialog.title',
                ...
                <Grid container spacing={1}>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <Typography variant="subtitle1" align="left">
                      <FormattedMessage id="templates.labels.teams" />
                    </Typography>
                  </Grid>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <FormControl
