...
export const FormTemplateUsers = ({
  open,
  handleCloseDialog,
  data: templateData,
  setNewData,
  oldData,
}: DialogFormProps<TemplateDataProps, UsersTemplateDataProps[]>) => {
  ...
  const { data } = useAxiosGet<UsersOrganizationDataProps[]>(
    `api/assigns/users-organizations/${templateData?.organization.id}/users`,
  )
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (formData) => {
    ...
    await api
      .request({
        method: 'POST',
        url: '/api/assigns/users-templates',
        data: formData,
      })
      .then((response) => {
        if (setNewData && oldData) {
          setNewData([
            ...oldData,
            {
              user_template_id: response.data.user_template_id,
      ...
      {open && (
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogTitle>{`${formatMessage({
            id: 'templates.users.dialog.title',
          })} -> ${templateData!.name}`}</DialogTitle>
          <DialogContent>
                ...
                <Grid container spacing={1}>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <Typography variant="subtitle1" align="left">
                      <FormattedMessage id="templates.labels.users" />
                    </Typography>
                  </Grid>
                  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                    <FormControl
